...
\subsection{total\_spend}
Outliers can significantly distort the shape of a distribution, leading to deviations from normality. In the case of total\_spend, the presence of 1.41\% outliers (1,410 records) that represent unusually high-value transactions may have a disproportionate influence on the overall distribution. \\
...
For total\_spend, this deviation is driven by a small number of extreme high-value purchases (1.41\% outliers) that stretch the data upward. For fulfillment\_time\_min, the data is highly symmetrical with only 0.81\% outliers but is cleanly truncated by a 1-minute physical minimum required to fulfill an order. Ultimately, these results perfectly reflect the real-world operational realities of the business: transaction values have no ceiling, and fulfillment times have a hard physical floor.

\chapter{Attempts to find normality in smaller groups}

\section{Comparison of fulfillment time of preparing espresso and brewd coffee}

To investigate if product complexity affects service consistency, the \texttt{fulfillment\_time\_min} variable was segmented into two groups: Single Espresso and Single Brewed Coffee. Table~\ref{tab:ks_small_groups_comparison} summarizes the Kolmogorov-Smirnov (K-S) test results.
...
\noindent To better show how the normality looks figures \ref{fig:brewed_coffee_dist} and \ref{fig:espresso_dist} show the graphical representation of the data.
...
\noindent Even though the Single espresso category didn't officially pass the test with p-value equal 0.447, we can observe a slight normality.\\
...
Here we are testing the normality for the averaged waiting time for the order between restaurants.
Using Kolmogorov-Smirnov test we recived the following results:
...
Here are also the graphs showing the graphical representation of the avaraged data distribution.
