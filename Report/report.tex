...
\subsection{Methodology}
To determine if satisfaction levels vary by ordering method, we analyzed the relationship between the \texttt{order\_channel} and \texttt{customer\_satisfaction} attributes[cite: 19, 419]. Because satisfaction is measured on a discrete ordinal scale (1-5) and the dataset exhibits non-normal distributions (as shown in Chapter 3), we utilized the non-parametric \textbf{Kruskal-Wallis H test}[cite: 179, 261]. A standard significance level of $\alpha = 0.05$ was applied.
